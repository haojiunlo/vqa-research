    ...
    def __init__(self, config, layer_id=0, is_cross_attention=False):
        ...
        if self.attention_type in ["global", "local"]:
            self.attention = GPTNeoSelfAttention(
                config, self.attention_type, is_cross_attention=False
            )
        elif self.is_cross_attention and self.attention_type in ["global", "local"]:
            self.attention = GPTNeoSelfAttention(
                config, self.attention_type, is_cross_attention=True
            )
        else:
...
class GPTNeoBlock(nn.Module):
    def __init__(self, config, layer_id):
        ...
        if config.add_cross_attention:
            self.crossattention = GPTNeoAttention(
                config, layer_id, is_cross_attention=True
            )
